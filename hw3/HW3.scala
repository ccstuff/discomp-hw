...
object HW3 {
	val inputFile = "/Users/yangmei/eclipse-workspace/hw/src/zzz/dblp1/"
	val conf = new SparkConf().setMaster("local[7]").setAppName("top").set("spark.executor.memory", "8g")
	...
	def top_words(): Unit ={
		val stopwords = sc.textFile("/Users/yangmei/eclipse-workspace/hw/src/WordCount/stopwords.txt").map(_.toLowerCase)
		val words = articles.select("title").map(row => row.getString(0).toLowerCase().dropRight(1).split(" "))
			.flatMap((t => t)).rdd.subtract(stopwords).map(t => (t,1)).reduceByKey((a,b)=>a+b)

		val arr = words.collect

		val top100words = arr.sortWith(_._2 > _._2).slice(0, 110)
		sc.parallelize(top100words.map(t => t._1)).coalesce(1).saveAsTextFile("/Users/yangmei/Desktop/2017-2018-1/discomp/hw3/words2")
		print("words done")
	}
